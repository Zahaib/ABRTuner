MediaPlayer.dependencies.BandwidthPredictor = function () {
	...
	if (lastHTTPRequest && lastRequested >= 0) {
	    ...
	    lastDownloadTime = (lastHTTPRequest.tfinish.getTime() - lastHTTPRequest.trequest.getTime()) / 1000; //seconds
	    ...
	    // debug
	    self.debug.log("--------Yun BWPredict lastChunk="+lastRequested+", downloadTime="+lastDownloadTime+"s, lastThroughput="+lastThroughput+"kb/s, lastChunkSize="+lastChunkSize);
	}
	...
	predictStd = function (lastRequested) {
		var self = this,
			bandwidthEst = 0,
			tmpIndex,
			tmpSum = 0,
			tmpDownloadTime = 0;
        self.debug.log("----------Yun StdPredict entering");
		var std_list = [];
		if (pastThroughput.length === 0) {
			return 0;
		} else {
			...
			var cnt=0;
			for (var i = tmpIndex; i<= lastRequested; i++) {
				tmpSum = tmpSum + pastThroughput[i];
				std_list.push(pastThroughput[i]);
				cnt+=1;
			}
			var average = tmpSum/parseFloat(cnt);
			var sumsqrdiff = 0;

			for (var k in std_list){
				var diff = k - average;
				var sqrdiff = diff*diff;
                sumsqrdiff = sumsqrdiff+sqrdiff;
			}
			var averagesumsqrdiff = sumsqrdiff/parseFloat(cnt);
			var stdDev = Math.sqrt(averagesumsqrdiff);

			self.debug.log("----------Yun StdPredict: stdEst="+ stdDev);
			return stdDev;
		}
	},
