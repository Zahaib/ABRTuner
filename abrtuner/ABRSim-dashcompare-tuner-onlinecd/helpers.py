...
from live_dash_syn_simulation_hyb_pen_performance_table import *
...
def isSane(bwArray, BR, stdbw, avgbw, size_pen):
  sanity = True
...
def updateSessionHistory(bitrate, clock, chunkid, CHUNKSIZE, sessionHistory, first_chunk, time_residue, attempt_id, completed, chunk_residue):
  ...
  if CHUNK_AWARE_MODE and bitrate in size_pen:
    bitrate = size_pen[bitrate][chunkid] * 8
...
def getBWFeaturesWeighted(CLOCK, sessionHistory, attempt_id, mode, chunkid, chunk_residue, bitrate, CHUNKSIZE):
  ...
  if chunk_residue > 0.0:
    total_curr_kbs = 0
    if CHUNK_AWARE_MODE and bitrate in size_pen:
      total_curr_kbs = size_pen[bitrate][chunkid] /1000.0
    else:
...
def chunksDownloaded(configsUsed, time_prev, time_curr, bitrate, bandwidth, chunkid, CHUNKSIZE, chunk_residue, usedBWArray, bwArray, time_residue, BLEN, sessionHistory, first_chunk, attempt_id,PLAYTIME,AVG_SESSION_BITRATE, margin, minCellSize, BUFFTIME, playerVisibleBW, chunk_when_last_chd_ran, p1_min, gradual_transition, additive_inc):
  ...
  if chunk_residue > 0 and time_prev + time2FinishResidueChunk <= time_curr:
    ...
    AVG_SESSION_BITRATE+=size_pen[bitrateAtIntervalStart][chunkid]
    ...
    if ch_detected:
      ...
      dict_name_backup = "live_dash_syth_hyb_pen_table_"+str(minCellSize)
    ...
    chunkid+=1
    #print time_curr/1000.0, 'HYB', bandwidth, (est_bandwidth), (est_std), round(p1_min,2), round(chunk_residue,2), round(BLEN,2), chunkid-1, bitrateAtIntervalStart, round(BUFFTIME,2)
    try:
      ...
      configsUsed.append((time_curr/1000.0, 'HYB', bandwidth, int(-1), int(-1), round(p1_min,2), round(chunk_residue,2), round(BLEN,2), chunkid-1, bitrateAtIntervalStart, round(BUFFTIME,2)))
    ...
    #else:
    #  print bitrateAtIntervalStart, chunkid-1, size_pen[bitrateAtIntervalStart][chunkid-1]
    #  AVG_SESSION_BITRATE+=size_pen[bitrateAtIntervalStart][chunkid-1]
...
def getRealBitrate(bitrate, chunkid, CHUNKSIZE):
  ...
  if CHUNK_AWARE_MODE and bitrate in size_pen and chunkid <len(size_pen[bitrate]):
    ret = size_pen[bitrate][chunkid]*8
  return ret
...
def parseSessionState(group):
  ...
  return ret, int(group.irow(0)["jointimems"]), int(group.irow(0)["playtimems"]), int(group.irow(0)["sessiontimems"]), int(group.irow(0)["lifeaveragebitratekbps"]), int(group.irow(0)["bufftimems"]), int(group.irow(0)["init_br"]), zip(ts,bw), int(group.irow(0)["chunkDuration"]), len(size_pen[ret[0]]) #10 , 75 # 
...
def parseSessionStateFromTrace(filename):
  ...
  jointimems = 0 #ts[0] + 1
  #init_br = bitrates[0]
  return bitrates, jointimems, totalTraceTime, totalTraceTime + jointimems, 1, 1, init_br, zip(ts,bw), chunkDuration, 64 #10 , 75 # 
