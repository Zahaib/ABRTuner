...
for initialBSM in [0.25]:
  ...
  #for discount in range(-10,201,10):
    ...
    candidateBR, jointime, playtimems, sessiontimems, bitrate_groundtruth, bufftimems, BR, bw_array, CHUNKSIZE, TOTAL_CHUNKS = parseSessionStateFromTrace(trace_file)
    ...
    while CLOCK < s.sessiontimems:
        ...
        if s.buffering and not s.sessionFullyDownloaded:
            ...
            if s.playStalled_thisInterval < s.interval / float(1000):  # chunk download so resume
                s.buffering = False
        #print CLOCK, interval, BR, BW, CHUNKS_DOWNLOADED, CHUNKSIZE,  chunk_residue, chunk_sched_time_delay, BLEN, session_history, first_chunk, ATTEMPT_ID
        ...
        #oldbw = BW
        s.BW = max(interpolateBWInterval(CLOCK, s.used_bw_array, s.bw_array),
