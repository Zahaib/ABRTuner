...
def performance_vector(sample, filename):
    ...
    for upri in range(1, 41, 10):
        ...
        for A in np.arange(0.01, 1.01, 0.04):
            ...
            candidateBR, jointime, playtimems, sessiontimems, bitrate_groundtruth, bufftimems, BR, bw_array, CHUNKSIZE, TOTAL_CHUNKS = parseSessionStateFromTrace(
                traceFile)

            #print "per: " + str(sample) + " " + str(len(bw_array))
            ...
            BLEN, CHUNKS_DOWNLOADED, CLOCK, chunk_residue, first_chunk, session_history = bootstrapSim(jointime, BW, BR,
                                                                                                      CHUNKSIZE)
            oldBR = BR
            buffering = False
		...
		#print "interval: " + str(interval) + " clock: " + str(CLOCK)
                ...
                if not sessionFullyDownloaded and chunk_sched_time_delay < interval:
                    numChunks, completionTimeStamps, chunk_sched_time_delay, session_history = chunksDownloaded(CLOCK - interval, CLOCK,
                                                                                               BR, BW,
                                                                                               CHUNKS_DOWNLOADED,
                                                                                               CHUNKSIZE, chunk_residue,
                                                                                               used_bw_array, bw_array,
                                                                                               chunk_sched_time_delay,
                                                                                               BLEN, session_history, first_chunk)
                ...
                #    session_history = updateSessionHistory(BR, CLOCK, CHUNKS_DOWNLOADED, CHUNKSIZE, session_history,
                ...
                if not first_chunk and not sessionFullyDownloaded and timeSinceLastDecision == decision_cycle:
                    if UTILITY_BITRATE_SELECTION:
                        ...
                        newBR = getUtilityBitrateDecision(BLEN, candidateBR, BW, CHUNKS_DOWNLOADED, CHUNKSIZE, BSM,
                                                          buffering_weight, session_history, chunk_residue, BR, CLOCK,
                                                          decision_cycle, bw_array, used_bw_array, sessiontimems, oldbw)
                ...
                #    BW = interpolateBWPrecisionServerStyle(CLOCK, BLEN, used_bw_array, bw_array)
                #else:
	#	if CLOCK == bw_array[-1][0]:
	#	    BW = bw_array[-1][1]
	#	else:
