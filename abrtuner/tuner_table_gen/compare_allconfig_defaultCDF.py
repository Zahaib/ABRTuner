...
for f in allconfig_f:
    ...
    for i in range(0,len(out)/step):
        ...
        if name.split(" ")[0] not in default_dict.keys():
            default_dict[name.split(" ")[0]] = dict()
        #if p2 not in default_dict[name.split(" ")[0]].keys():
        default_dict[name.split(" ")[0]][p2] = (avgbr, rebuf, p2)
        #if p2==0.25:
        #if p1 ==0.5 and p2==41:
            #print p1, p2, avgbr, rebuf, name
...
for session in compare.keys():
    if len(compare[session])!=1:
        ...
        print session, 
        print compare[session][0],
        for c in default_dict[session].keys():
              print default_dict[session][c],
        print ""
