...
#def chunksDownloaded(configsUsed, time_prev, time_curr, bitrate, bandwidth, chunkid, CHUNKSIZE, chunk_residue, used_bw_array, bw_array, time_residue, BLEN, session_history, first_chunk, attempt_id,PLAYTIME,AVG_SESSION_BITRATE, minCellSize, BUFFTIME, player_visible_bw, chunk_when_last_chd_ran, p1_min, gradual_transition, additive_inc, bandwidthEsts, pastErrors, windowSize, change_magnitude, discount, max_error, bola_gp, bola_vp, buffer_target_s):
#  chunkCount = 0.0
...
#    time_prev += time2FinishResidueChunk + nextChunkDelay
#    ## update session_history here, since a chunk finished download
#    session_history = updateSessionHistory(bitrateAtIntervalStart, completionTime, chunkid, CHUNKSIZE, session_history, first_chunk,
...
#    if ONCD:
#      CDinterval = 5
#      ch_detected, ch_index = onlineCD(session_history, chunk_when_last_chd_ran, CDinterval, player_visible_bw)
#      est_bandwidth, est_std = getBWFeaturesWeightedPlayerVisible(player_visible_bw, ch_index)
#      nsteps = 2.0
#      p1_min_new = 0.0
#      if ch_detected:
...
#    
#    ######## MPC code
#    future_bandwidth, max_error, bandwidthEsts, pastErrors = getMPCBW(session_history, bandwidthEsts, pastErrors, chunkid, discount)
...
#    bitrateDASH = getUtilityBitrateDecision_dash(session_history, chunkid, chunkid+1, BLEN+CHUNKSIZE, p1_min)
#    chunkid+=1
#    
#
#    ############ MPC bitrate ############
...
#    if CHUNK_AWARE_MODE:
#      bitrate = getRealBitrate(bitrateAtIntervalStart, chunkid, CHUNKSIZE)
#    bandwidth = max(interpolateBWInterval(time_prev, used_bw_array, bw_array),0.01)
#    attempt_id += 1
#    time2DownloadFullChunk = (bitrate /float(bandwidth))
#     
#  # loop untill chunks can be downloaded in the interval, after each download add random delay
