...
def make_request_handler(input_dict):
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            new_quality = tuner_logic.getUtilityBitrateDecision_dash(bandwidthEst, \
            	                                                 lastChunkID + 1, \
            	                                                 bufferLen, \
            	                                                 self.input_dict['beta'])

            if new_quality < self.input_dict['quality']:
              self.input_dict['switchLock'] = 5
            else:
              self.input_dict['switchLock'] -= 1

            if new_quality > self.input_dict['quality'] and self.input_dict['switchLock'] <= 0 or new_quality < self.input_dict['quality']:
              self.input_dict['quality'] = new_quality

            print lastChunkID, new_quality, self.input_dict['quality'], self.input_dict['switchLock'], self.input_dict['beta']
            end_of_video = False
            if ( lastChunkID == TOTAL_VIDEO_CHUNKS ):
                ...
                self.input_dict['beta'] = 0.25
                self.input_dict['switchLock'] = 0
                self.input_dict['quality'] = 0


            send_data = str(self.input_dict['quality'])
