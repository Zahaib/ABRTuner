...
def getXput(chunks_dash):
    total_size = 0
    total_time = 0
    total_time_avoid_buff = 0
    chunk_level_delta_xput = []
    for ID in sorted(chunks_dash.keys()):
        total_size += chunks_dash[ID]['size']
        total_time += chunks_dash[ID]['end'] - chunks_dash[ID]['start']
        total_time_avoid_buff += (chunks_dash[ID]['end'] - chunks_dash[ID]['start']) - chunks_dash[ID]['buffering_delay'] * 1000.0
        if chunks_dash[ID]['buffering_delay'] > 0:
            # print ID, chunks_dash[ID]['end'] - chunks_dash[ID]['start'], chunks_dash[ID]['buffering_delay'], chunks_dash[ID]['buffering_delay'] * 1000.0
            chunk_xput = chunks_dash[ID]['size'] * 8 / float(chunks_dash[ID]['end'] - chunks_dash[ID]['start'])
            needed_xput = chunks_dash[ID]['size'] * 8 / float((chunks_dash[ID]['end'] - chunks_dash[ID]['start']) - chunks_dash[ID]['buffering_delay'] * 1000.0)
            delta_xput_chunk = needed_xput - chunk_xput
            # print delta_xput_chunk, delta_xput_chunk / chunk_xput 
            chunk_level_delta_xput.append(delta_xput_chunk / chunk_xput)

    overall_actual_xput = (total_size * 8) / total_time
    overall_needed_xput = (total_size * 8) / total_time_avoid_buff
    session_delta_xput = overall_needed_xput - overall_actual_xput #/ float(overall_actual_xput)
    # print overall_actual_xput,overall_needed_xput, session_delta_xput, (overall_needed_xput - overall_actual_xput) / overall_actual_xput
    return overall_actual_xput, overall_needed_xput, session_delta_xput / overall_actual_xput, chunk_level_delta_xput
...
for file_name in file_names:
    ...
    for line in f:
        ...
        if "chunk" in chunk[0]:
            ...
            chunks_dash[ID]["buffering_delay"] = buff_delay_found
        ...
        if "buffering" in chunk[0]:
            ...
            if ID > 1:
                buff_delay_found = end - start
        else:
            buff_delay_found = 0        
                # chunks_dash[ID]["buffering_delay"] = end - start 
                # print >> sys.stderr, start, play, bufferlen, end, ID, end - start
    ...
    if name not in dash_QoE.keys():
        dash_QoE[name] = dict()
    if name not in dash_QoE_chunks_dash.keys():
        dash_QoE_chunks_dash[name] = dict()
    if scheme not in dash_QoE_chunks_dash[name].keys():
        dash_QoE_chunks_dash[name][scheme] = dict()

    dash_QoE_chunks_dash[name][scheme]['summary'] = (avgBitrate,rebufRatio, len(chunks_dash), QoE, total_quality_change/float(num_chunks_video))
    dash_QoE_chunks_dash[name][scheme]['raw'] = chunks_dash
...
session_delta_xput_cdf = []
chunk_delta_xput_cdf = []
for name in dash_QoE_chunks_dash.keys():
    if dash_QoE_chunks_dash[name]['pensieve-pensvid']['summary'][1] > 0:
        sess_actual_xput, sess_needed_xput, session_delta, chunk_delta = getXput(dash_QoE_chunks_dash[name]['pensieve-pensvid']['raw'])
        session_delta_xput_cdf.append(session_delta)
        chunk_delta_xput_cdf += chunk_delta
        # if sess_needed_xput == sess_actual_xput:
        #     print dash_QoE_chunks_dash[name]['pensieve-pensvid']['summary']


f_out = open(output_dir + "/cdf_chunk_delta_xput.txt",'w')
for i in range(0,101):
    f_out.write(str(i)+" "+str(np.percentile(chunk_delta_xput_cdf, i))+"\n")
f_out.close()
f_out = open(output_dir + "/cdf_session_delta_xput.txt",'w')
for i in range(0,101):
    f_out.write(str(i)+" "+str(np.percentile(session_delta_xput_cdf, i))+"\n")
f_out.close()
