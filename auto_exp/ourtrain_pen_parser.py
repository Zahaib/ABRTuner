...
if len(sys.argv) < 4:
  print >> sys.stderr, "Incorrect usage...\nUsage: python " + sys.argv[0] + " <path to log dir> <path to output dir> <bw limist>"
  sys.exit()
...
for file_name in file_names:
    ...
    if "pensieve-pensvid" in file_name:
...
#      print ID, chunks_dash[ID]['bitrate']
    ...
    print name, scheme
    if rebufRatio > 0 and scheme == 'robustmpc':
...
for i in dash_QoE.keys():
    ...
    for ii in dash_QoE[i].keys():
        ...
        if ii not in change_cdf.keys():
            change_cdf[ii] = []            
        if ii != "pensieve-pensvid" and ii not in rebuf_per_cdf.keys():
            rebuf_per_cdf[ii] = []
        if ii != "pensieve-pensvid" and ii not in bitrate_per_cdf.keys():
            bitrate_per_cdf[ii] = []
        if ii != "pensieve-pensvid" and ii not in qoe_per_cdf.keys():
            qoe_per_cdf[ii] = []
        if ii != "pensieve-pensvid" and ii not in change_per_cdf.keys():
            change_per_cdf[ii] = []
        ...
        #if ii == 'pensieve-pensvid' and float(dash_QoE[i][ii][4]) < 100:
        ...
        if ii != "pensieve-pensvid":
            bitrate_per_diff = 100*(float(dash_QoE[i]["pensieve-pensvid"][0]) - float(dash_QoE[i][ii][0]))/float(dash_QoE[i][ii][0])
            bitrate_per_cdf[ii].append(bitrate_per_diff)
            rebuf_per_diff = 100*(float(dash_QoE[i][ii][1]) - float(dash_QoE[i]["pensieve-pensvid"][1]))
          
            if ii == 'robustmpc' and rebuf_per_diff > 0.0:
              n = '_'.join(orig_file_name[i].split("_")[1:6]) + ".txt"
              print "../trace_500_pen/" + n, round(dash_QoE[i]["pensieve-pensvid"][1],2), round(dash_QoE[i][ii][1],2)
            rebuf_per_cdf[ii].append(rebuf_per_diff)
            #try:
            qoe_diff = 100*(float(dash_QoE[i]["pensieve-pensvid"][3]) - float(dash_QoE[i][ii][3]))/abs(float(dash_QoE[i][ii][3]))
            if ii == 'pensieve-pensvid':
              if qoe_diff > 0:
                qoe_imp_pens['imp'].append(qoe_diff) 
                print i, dash_QoE[i][ii], dash_QoE[i]["pensieve-pensvid"]
              else:
                qoe_imp_pens['red'].append(qoe_diff)
            
            #  print >> sys.stderr, dash_QoE[i]["pensieve-pensvid"][3], dash_QoE[i][ii][3]
            qoe_per_cdf[ii].append(qoe_diff)
            #except ZeroDivisionError:
            #  pass
            #if 100*(float(dash_QoE[i]["pensieve-pensvid"][3]) - float(dash_QoE[i][ii][3]))/float(dash_QoE[i][ii][3]) < -30:
            #    print >> sys.stderr, i, dash_QoE[i]["pensieve-pensvid"][3], dash_QoE[i][ii][3], float(dash_QoE[i]["pensieve-pensvid"][3]) - float(dash_QoE[i][ii][3])
            if float(dash_QoE[i][ii][4]) == 0:
                change_per_cdf[ii].append(100*(float(dash_QoE[i][ii][4]) - float(dash_QoE[i]["pensieve-pensvid"][4]))/0.01)
            else:	
                change_per_cdf[ii].append(100*(float(dash_QoE[i][ii][4]) - float(dash_QoE[i]["pensieve-pensvid"][4]))/float(dash_QoE[i][ii][4]))
...
#   try:
#     percentage_diff = (average_QoE[name]["pensieve-pensvid"] - average_QoE[name]["pensieve-pensvid"]) / average_QoE[name]["pensieve-pensvid"] * 100
...
#   try:
#     if file_name_dict[fname]['pensieve-pensvid'][1] < file_name_dict[fname]["pensieve-pensvid"][1]:
#       print fname, file_name_dict[fname]['pensieve-pensvid'][1], file_name_dict[fname]["pensieve-pensvid"][1]
